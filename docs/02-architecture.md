...
- **Reproducible dev shell ([Nix flake](../flake.nix)):** `nix develop` yields the whole toolchain — Node 22 LTS, `rsvg-convert` (for `og:image`), and Playwright's Chromium bundled and auto-patched by nixpkgs — on `x86_64-linux`, `aarch64-linux`, and `aarch64-darwin`. The shell hook exports `PLAYWRIGHT_BROWSERS_PATH` to the nix-provided browsers, so `test:e2e` needs no `npx playwright install` and no `LD_LIBRARY_PATH`. `flake.lock` pins the nixpkgs revision; run the full battery inside the shell to verify a toolchain change (`nix develop -c bash -c 'npm ci && npm run lint && … && npm run test:e2e'`). **Maintenance note:** if `nix flake update` bumps nixpkgs' Playwright chromium revision, `test:e2e` breaks until the npm `playwright` package is realigned to nixpkgs' version — see the inline comment in `flake.nix`. Intel Mac (`x86_64-darwin`) is unsupported (no nixpkgs binary).
