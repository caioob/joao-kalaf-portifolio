...
| Detail hero | New hero (the project cover, `slot="thumbnail"` so it's the same variants as the card) inside the detail, under the title block and above the description; the rest of the detail content cross-fades in as the transition's root group. |
